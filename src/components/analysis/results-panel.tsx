...
const getParticleColor = (confidence: number) => {
    if (confidence > 0.9) return 'rgba(255, 0, 0, 0.9)'; // High confidence - red
    if (confidence > 0.75) return 'rgba(255, 165, 0, 0.8)'; // Medium - orange
    return 'rgba(255, 255, 0, 0.7)'; // Low - yellow
};
        ...
        <Card className="h-full flex flex-col shadow-lg">
            ...
            <CardContent className="flex-1 flex flex-col gap-4">
                {isLoading ? (
                    ...
                    <>
                        <div className="relative w-full aspect-video rounded-lg overflow-hidden border bg-gray-200">
                            <Image src={image} alt="Analyzed water sample" fill style={{ objectFit: 'contain' }} />
                            {particles.map((p, i) => (
                                <div
                                    key={i}
                                    className="absolute rounded-full w-2.5 h-2.5 border border-white/70"
                                    style={{
                                        left: `${p.x * 100}%`,
                                        top: `${p.y * 100}%`,
                        ...
                        <div className="grid grid-cols-2 gap-4 text-center">
                            <div className="p-4 bg-secondary/70 rounded-lg">
                                <FlaskConical className="mx-auto h-6 w-6 text-primary mb-2" />
                                <p className="text-2xl font-bold">{particles.length}</p>
                                <p className="text-sm text-muted-foreground">Particles Detected</p>
                            </div>
                            <div className="p-4 bg-secondary/70 rounded-lg">
                                <BarChart3 className="mx-auto h-6 w-6 text-accent mb-2" />
                                <p className="text-2xl font-bold">~{(particles.length / 0.5).toFixed(1)}</p>
                                <p className="text-sm text-muted-foreground">Particles/Liter (Est.)</p>
                            </div>
                        </div>
                        
                        <h3 className="font-semibold mt-2">AI Analysis Summary</h3>
                        <ScrollArea className="h-32 p-3 border rounded-md bg-secondary/70 text-sm text-foreground">
                    ...
                    <div className="flex-1 flex flex-col justify-center items-center text-center text-muted-foreground p-8 border-2 border-dashed rounded-lg bg-secondary/40">
